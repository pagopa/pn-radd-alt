...
class ActServiceTest  {
    ...
    void testActInquiryWhenControlCheckArrResponseError() {
        ...
        ExpectedLoggingAssertions.assertThat(logging).hasErrorMessage("[AUD_RADD_ACTINQUIRY] FAILURE - End ACT Inquiry with error Iun not found with params - uid=test cxId=123 cxType=PF taxCode=test recipientInternalId=ABCDEF12G34H567I iun= status=ActInquiryResponseStatus(code=99, message=Iun not found with params)");
    }
    ...
    void testActInquiryWhenNotificationIsCancelled() {
        ...
        ExpectedLoggingAssertions.assertThat(logging).hasInfoMessage("[AUD_RADD_ACTINQUIRY] BEFORE - Start ACT Inquiry - uid=test cxId=123 cxType=PF taxCode=test iun=");
        ExpectedLoggingAssertions.assertThat(logging).hasErrorMessage("[AUD_RADD_ACTINQUIRY] FAILURE - End ACT Inquiry with error Questa notifica è stata annullata dall’ente mittente - uid=test cxId=123 cxType=PF taxCode=test recipientInternalId=ABCDEF12G34H567I iun=iun status=ActInquiryResponseStatus(code=80, message=Questa notifica è stata annullata dall’ente mittente)");
    }
    ...
    void testActInquiryWhenNotificationHasNotDocumentsAvailable() {
        ...
        ExpectedLoggingAssertions.assertThat(logging).hasInfoMessage("[AUD_RADD_ACTINQUIRY] BEFORE - Start ACT Inquiry - uid=test cxId=123 cxType=PF taxCode=test iun=");
        ExpectedLoggingAssertions.assertThat(logging).hasErrorMessage("[AUD_RADD_ACTINQUIRY] FAILURE - End ACT Inquiry with error Documenti non disponibili - uid=test cxId=123 cxType=PF taxCode=test recipientInternalId=ABCDEF12G34H567I iun=iun status=ActInquiryResponseStatus(code=4, message=Documenti non disponibili)");
    }
