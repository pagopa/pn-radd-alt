...
class RaddRegistryImportDaoImplTest extends BaseTest.WithLocalStack {
    ...
    void testGetRegistryImportByCxIdAndRequestIdFilterByStatus() {
        StepVerifier.create(raddRegistryImportDAO.getRegistryImportByCxIdAndRequestIdFilterByStatus(baseEntity.getCxId(), baseEntity.getRequestId(), ImportStatus.TO_PROCESS))
                .expectNextMatches(entity -> entity.getCxId().equals(baseEntity.getCxId()) && entity.getRequestId().equals(baseEntity.getRequestId()) && entity.getStatus().equals(ImportStatus.TO_PROCESS.name()));
    }
